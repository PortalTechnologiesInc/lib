...
impl SubscriptionState {
    ...
    pub async fn decrement_and_check_if_zero(&self) -> bool {
        ...
        if inner.count > 0 {
            ...
            log::warn!(
                "Attempted to decrement count below zero for subscription id {}",
                inner.id
            );
        }
        inner.count == 0
...
impl MessageRouterState {
    ...
    pub(crate) async fn handle_relay_pool_notification<C: Channel>(
        &mut self,
        channel: &Arc<C>,
        ...
        C::Error: From<nostr::types::url::Error>,
    {
        ...
        for id in other_conversations {
            self.dispatch_event(channel, id, message.clone()).await?;
        }
        Ok(())
    }
        ...
        C::Error: From<nostr::types::url::Error>,
    {
        ...
        for (conversation_id, conv_state) in self.conversations.iter_mut() {
            if !conv_state.check_subscription_id(&subscription_id).await {
                continue;
            }
