...
- **portal_wallet.supported_units** — unit name = keyset size (e.g. `sat = 32`, `vip = 32`)
- **portal_wallet.unit_info.<unit>** — `title`, `description`, `show_individually` (false = fungible, true = tickets), optional `front_card_background`, `back_card_background`, `[portal_wallet.unit_info.<unit>.kind.Event]` with `date`, `location`
